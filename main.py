...
if __name__ == "__main__":
    file_paths = [
        "cg_btc.csv", "cg_eth.csv", "cg_ltc.csv", "cg_xrp.csv",
        "cg_bnb.csv", "cg_bch.csv", "cg_link.csv", "cg_etc.csv"
    ]

    m_values = range(1, 25)
    ell_values = range(1, 25)
    threshold = 0.5

    slider_heatmap.timelapse_heatmap(
        file_paths=file_paths,
        m_values=m_values,
        ell_values=ell_values,
        chunk_size=100,   # e.g. 200 days per chunk
        stride=30,        # move forward by 50 days each time
        threshold=threshold
    )
...
if __name__ == "__main__":
    ...
    file_paths = ["cg_btc2019.csv", "cg_eth2019.csv", "cg_ltc2019.csv", "cg_xrp2019.csv", "cg_bnb2019.csv", "cg_bch2019.csv", "cg_link2019.csv", "cg_etc2019.csv"]

    m_values = range(1, 20)
    ell_values = range(1, 9)
    threshold = 0.5

    #Debug to test to see if all windows are being used - so far none ar skipped.
    rolling_heatmap.debug_window_usage(file_paths, m_values, ell_values)
    ...
    try:
        ...
        #try to do more than 1 step ahead
        #next_log_return = single_step_forecast.predict_ell_steps_beyond_last_snapshot(X1, Phi, Lambda, ell=2)
