...
def debug_window_usage(file_paths, m_values, ell_values):
    """
    For each (m, ell), prints how many total windows are possible and
    how many were skipped due to LinAlgError or shape issues.
    """
    ...
    for m in m_values:
        for ell in ell_values:
            ...
            for t in range(0, T - m - ell + 1):
                X1 = data_matrix[:, t : t + m - 1]
                X2 = data_matrix[:, t + 1 : t + m]
                # Check if there's enough data in each slice
                if X1.shape[1] < 1:
                    continue

                total_possible += 1
                try:
                    # If DMD fails, we catch LinAlgError
                    dmd_functions.dmd_decomposition_rolling(X1, X2)
                    used_windows += 1
                except np.linalg.LinAlgError:
                    skipped_windows += 1

            # Print summary for this (m, ell)
            print(f"(m={m}, ell={ell}): total_possible={total_possible}, "
                  f"used_windows={used_windows}, skipped={skipped_windows}")